...
describe("English product copy", () => {
  it("keeps the agent handoff and mission positioning explicit", () => {
    ...
    expect(COPY.effortTitle).toBe("2 · Choose effort")
    expect(COPY.handoffTitle).toBe("3 · Copy to ChatGPT")
    expect(COPY.constraintsTitle).toBe("Review and edit needs")
    ...
    expect(COPY.promptResultProtocol).toContain(`[Open updated ${PRODUCT_NAME} plan]`)
    ...
    expect(COPY.agentHint).toContain(`open ${PRODUCT_NAME} in Work`)
    ...
    expect(PRODUCT_URL).toBe("https://daymaker.fun")
